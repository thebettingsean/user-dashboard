...
const SPORT: League = 'cfb'

function getDateRangeForSport() {
  const start = new Date()
  start.setHours(0, 0, 0, 0)
  const end = new Date(start)
  end.setDate(end.getDate() + 7) // CFB games are weekly
...
async function loadScriptMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[CFB snapshot-refresh] Failed to load script meta', error)
    return meta
  }
...
async function loadPickMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[CFB snapshot-refresh] Failed to load pick meta', error)
    return meta
  }
...
/**
 * GET /api/cron/refresh-cfb-snapshots
 * Automated cron job to refresh CFB game snapshots
 * Runs every 2 hours
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`📝 Upserting ${payloads.length} CFB game snapshots to college_game_snapshots...`)

    // Upsert to college_game_snapshots table
    const { error } = await snapshotsClient
      .from('college_game_snapshots')
      .upsert(payloads, { onConflict: 'sport,game_id' })

    if (error) {
      console.error(`❌ Failed to upsert CFB snapshots:`, error)
      return NextResponse.json(
        { error: error.message, sport: 'CFB' },
        { status: 500 }
      )
    }

    console.log(`✅ Successfully refreshed ${payloads.length} CFB game snapshots`)
    console.log('🏈 ========== CFB SNAPSHOTS REFRESH COMPLETE ==========\n')
    ...
    console.error('❌ Unexpected error in CFB cron job:', error)
    return NextResponse.json(
      { error: error.message || 'Unexpected error', sport: 'CFB' },
      { status: 500 }
    )
  }
}
