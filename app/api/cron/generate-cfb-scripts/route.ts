...
/**
 * CFB-specific cron job to pre-generate AI scripts
 * Runs every 2 hours during college football season
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('\n🏈 ========== CFB SCRIPT GENERATION CRON START ==========')
    ...
    if (allGames.length === 0) {
      console.log('⚠️ No CFB games found today')
      return NextResponse.json({
        success: true,
        message: 'No CFB games to process',
    ...
    // OPTIMIZATION: Fetch ALL existing CFB scripts at once (single DB query)
    ...
    if (existingScripts) {
      ...
      console.log(`📝 Found ${existingScripts.length} fresh CFB scripts (< 1 hour old)`)
    }

    // Process all CFB games
    for (const game of allGames) {
      const gameId = game.game_id || game.gameId
      const sport = 'CFB'

      try {
        console.log(`🎯 Checking script for CFB game: ${gameId}`)
