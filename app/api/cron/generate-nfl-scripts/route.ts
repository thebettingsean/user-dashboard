...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('\n🏈 ========== NFL SCRIPT GENERATION CRON START ==========')
    ...
    const allGames = (gamesData.games || []).filter((game: any) => game.sport === 'NFL')
    
    console.log(`📊 Found ${allGames.length} NFL games to process`)
    
    if (allGames.length === 0) {
      console.log('⚠️ No NFL games found today')
      return NextResponse.json({
        success: true,
        message: 'No NFL games to process',
    ...
    // OPTIMIZATION: Fetch ALL existing NFL scripts at once (single DB query)
    ...
    if (existingScripts) {
      ...
      console.log(`📝 Found ${existingScripts.length} fresh NFL scripts (< 1 hour old)`)
    }

    // Process all NFL games
    for (const game of allGames) {
      const gameId = game.game_id || game.gameId
      const sport = 'NFL'

      try {
        console.log(`🎯 Checking script for NFL game: ${gameId}`)
