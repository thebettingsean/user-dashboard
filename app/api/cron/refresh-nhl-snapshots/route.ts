...
const SPORT: League = 'nhl'

function getDateRangeForSport() {
  const start = new Date()
  start.setHours(0, 0, 0, 0)
  const end = new Date(start)
  end.setDate(end.getDate() + 2) // NHL games are frequent
...
async function loadScriptMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[NHL snapshot-refresh] Failed to load script meta', error)
    return meta
  }
...
async function loadPickMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[NHL snapshot-refresh] Failed to load pick meta', error)
    return meta
  }
...
/**
 * GET /api/cron/refresh-nhl-snapshots
 * Automated cron job to refresh NHL game snapshots
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (games.length === 0) {
      console.log(`⏭️  No NHL games to process`)
    ...
    console.log(`📝 Upserting ${payloads.length} NHL game snapshots to game_snapshots...`)
    ...
    if (error) {
      console.error(`❌ Failed to upsert NHL snapshots:`, error)
      return NextResponse.json(
        { error: error.message, sport: 'NHL' },
        { status: 500 }
      )
    }

    console.log(`✅ Successfully refreshed ${payloads.length} NHL game snapshots`)
    console.log('🏒 ========== NHL SNAPSHOTS REFRESH COMPLETE ==========\n')
    ...
    console.error('❌ Unexpected error in NHL cron job:', error)
    return NextResponse.json(
      { error: error.message || 'Unexpected error', sport: 'NHL' },
      { status: 500 }
    )
  }
}
