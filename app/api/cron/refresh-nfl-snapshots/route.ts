...
function getDateRangeForSport() {
  ...
  end.setDate(end.getDate() + 7) // NFL games are weekly
...
async function loadScriptMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[NFL snapshot-refresh] Failed to load script meta', error)
    return meta
  }
...
async function loadPickMeta(gameIds: string[]) {
  ...
  if (error) {
    console.error('[NFL snapshot-refresh] Failed to load pick meta', error)
    return meta
  }
...
/**
 * GET /api/cron/refresh-nfl-snapshots
 * Automated cron job to refresh NFL game snapshots
 * Runs every 1 hour
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (games.length === 0) {
      console.log(`⏭️  No NFL games to process`)
    ...
    console.log(`📝 Upserting ${payloads.length} NFL game snapshots to game_snapshots...`)
    ...
    if (error) {
      console.error(`❌ Failed to upsert NFL snapshots:`, error)
      return NextResponse.json(
        { error: error.message, sport: 'NFL' },
        { status: 500 }
      )
    }

    console.log(`✅ Successfully refreshed ${payloads.length} NFL game snapshots`)
    console.log('🏈 ========== NFL SNAPSHOTS REFRESH COMPLETE ==========\n')
    ...
    console.error('❌ Unexpected error in NFL cron job:', error)
    return NextResponse.json(
      { error: error.message || 'Unexpected error', sport: 'NFL' },
      { status: 500 }
    )
  }
}
