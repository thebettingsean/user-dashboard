...
const ESPN_SITE_URL = 'https://site.api.espn.com/apis/site/v2/sports/basketball/nba'
...
export async function GET(request: Request) {
  ...
  if (!gameId) {
    return NextResponse.json(
      { error: 'gameId parameter is required. Example: ?gameId=401704949' },
      { status: 400 }
    )
  }

  try {
    console.log(`[NBA Box Score Explorer] Fetching game ${gameId}`)
    ...
    for (const competitor of competitors) {
      ...
      console.log(`[NBA Explorer] Fetching stats for ${teamName} (${teamId})`)
    ...
    if (teamStats.length > 0) {
      ...
      const scoringCategory = firstTeam.statistics?.splits?.categories?.find(
        (cat: any) => cat.name === 'scoring'
      )

      if (scoringCategory?.athletes?.[0]?.athlete?.$ref) {
        const athleteId = scoringCategory.athletes[0].athlete.$ref.split('/athletes/')[1]?.split('?')[0]
        
        if (athleteId) {
          console.log(`[NBA Explorer] Fetching sample player stats for athlete ${athleteId}`)
    ...
    console.error('[NBA Box Score Explorer] Error:', error)
    return NextResponse.json(
      { 
        error: error.message,
        game_id: gameId,
