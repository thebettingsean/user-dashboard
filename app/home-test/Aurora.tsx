...
export default function Aurora(props: AuroraProps) {
  const { 
    colorStops = ['#3b82f6', '#60a5fa', '#8b5cf6'], 
    amplitude = 1.0, 
    blend = 0.5,
    speed = 0.5
  } = props
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!Renderer || !Program || !Mesh || !Color || !Triangle) return
      ...
      const update = () => {
        ...
        const elapsed = (t - startTime) * 0.001 // Convert to seconds
        const { speed: currentSpeed = speed } = propsRef.current

        // The shader uses uTime * 0.1 and uTime * 0.25, so we multiply by a factor to make animation visible
